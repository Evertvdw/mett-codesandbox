...
import { IComponentContainer, ITheme, IThemePark, Theme, ThemeStates } from "./types";
...
export function createThemePark(app: any | VueConstructor<Vue>, store: Store<object>, router: VueRouter): IThemePark {
	...
	const getComponents = function(): Promise<IComponentContainer | null> {
		return new Promise((resolve, reject) => {
			...
			if (_themeState == ThemeStates.loaded) {
				resolve(_components);
			} else if (_themeState == ThemeStates.loading || _themeState == ThemeStates.initial) {
	...
	const setComponentsByTheme = function(theme: ITheme) {
		...
		if (typeof theme.failOver != "undefined" && theme.failOver != null) setComponentsByTheme(theme.failOver);
	...
	const getComponentsByContainer = function(componentsContainer: any, prefix?: string) {
		...
		for (const componentName in componentsContainer) {
			...
			if (typeof component == "object" && typeof component.render == "undefined") {
				Object.assign(components, getComponentsByContainer(component, componentId));
	...
	return {
		...
		loadErrorTheme(testThemes?: boolean) {
			...
			log({
				short_message: "[_ID_] | Loading error theme",
				full_message: "",
				level: 7
			});
		...
		loadTheme(targetTheme: string, testThemes?: boolean) {
			...
			if (testThemes) {
				return new Promise((resolve, reject) => {
					import("src/mett/theming/__tests__/themes/_initializers/" + targetTheme).then(
						function(result) {
			...
			log({
				short_message: `[_ID_] | Loading theme ${_themeToLoad}`,
				full_message: "",
				level: 7
			});
			return new Promise((resolve, reject) => {
		...
		getComponent(componentLocation: string, key: string, asyncPayload?: any): Promise<VueConstructor<Vue>> {
			...
			return new Promise(async (resolve, reject) => {
				...
				componentLocationParts.push(componentLocationParts[componentLocationParts.length - 1]);
				...
				if (!(await this.componentExists(componentLocation))) {
					store.dispatch("error/addError", {
						...
						title: app.i18n.t("errors.unableToLoadComponentOrSubComponent.title"),
						message: app.i18n.t("errors.unableToLoadComponentOrSubComponent.description", [
							componentLocation,
							componentSubLocation,
							_themeName
						])
					});
					return reject();
				...
				if (
					...
					try {
						...
						if (
							...
							currentComponent.options.asyncLoadExecutionKeys.indexOf(asyncLoadExecutionKey) == -1
						) {
							...
							currentComponent.options.asyncLoadExecutionKeys.push(asyncLoadExecutionKey);

							currentComponent.options.asyncLoadExecutions.push(
								currentComponent.options.methods
									.asyncLoad(Object.assign({ vm: _vue, store, router }, asyncPayload))
									.then(() => {
						...
						await currentComponent.options.asyncLoadExecutions[
							currentComponent.options.asyncLoadExecutionKeys.indexOf(asyncLoadExecutionKey)
						];
					} catch (error) {
						currentComponent.options.asyncLoadExecutionKeys = null;
						currentComponent.options.asyncLoadExecutions = null;
		...
		async componentExists(componentLocation: string) {
			...
			if (currentComponents && (currentComponents[componentLocation] || currentComponents[componentSubLocation]))
				return true;
