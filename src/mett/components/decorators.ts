...
export function Action(actionPath: string) {
	return createDecorator((options, key) => {
		...
		options.methods[key] = function(payload?: any, dispatchOptions?: DispatchOptions) {
			return (this.$store as Store<any>).dispatch(actionPath, payload, dispatchOptions);
		};
	});
}
			...
			componentOptions.beforeCreate = function(this: Vue) {
				...
				if (typeof componentLocation == "string") {
					this.$options.components[k] = function() {
						return theThis.$root.$themePark.getComponent(componentLocation, k, payload);
					};
				} else {
